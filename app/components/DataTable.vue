...
const getRowClass = (row: any, rowClass?: string | ((row: any) => string)) => {
  if (!rowClass) return ''
...
<template>
  <div v-if="loading" class="flex flex-col items-center justify-center py-16 text-gray-400 dark:text-gray-500">
    ...
    <p class="text-sm">{{ loadingText ?? 'Carregando...' }}</p>
  </div>
        ...
        <tr v-if="data.length === 0">
          <td :colspan="columns.length" class="px-4 py-16 text-center">
            <div class="flex flex-col items-center gap-2 text-gray-400 dark:text-gray-500">
              <UIcon :name="emptyIcon ?? 'i-lucide-inbox'" class="text-3xl" />
              <p class="text-sm">{{ emptyText ?? 'Nenhum registro encontrado' }}</p>
            </div>
          </td>
        </tr>
        <tr
